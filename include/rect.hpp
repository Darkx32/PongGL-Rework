...
SDL_AppResult Rect::init(glm::vec2, glm::vec2, glm::vec3)
{
...
SDL_AppResult Rect::render()
{
    ...
    transform = glm::scale(transform, glm::vec3(this->scale, 0.0f));
    transform *= projection; // project is created in global.hpp header

    glUseProgram(this->shader.program);
    if (glGetError() != GL_NO_ERROR)
    {
        glGetProgramInfoLog(this->shader.program, 512, nullptr, this->infoLog);
        SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Error to use program: %s", this->infoLog);
    }
